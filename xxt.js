...
// @namespace    aite
// @version      1.0
// @description  支持视频、音频、文档、图书自动完成，章节测验考试自动答题提交，支持自动切换任务点、挂机阅读时长、自动挂机看尔雅MOOC，自动登录等，解除各类功能限制
// @author       fenzai & wd & Shadow
// @match        *://*.chaoxing.com/*
// @match        *://*.edu.cn/*
// @connect      user.902000.xyz
...
var setting = {
       ...
       //你是我的眼，终有一天你会有出头之日的！
        ...
        ,rate: '2' // 视频播放默认倍率，参数范围0∪[0.0625,16]，'0'为秒过，默认'2'倍
...
function beforeFind() {
    ...
    setting.div = $(
        '<div style="border: 2px dashed rgba(255,12,0,0.7); width: 330px; position: fixed; top: 0; right: 0; z-index: 99999; background-color: rgba(255,250,110,0.8); overflow-x: auto;">' +
        '<span style="font-size: medium;"></span>' +
         '<label style="font-weight: "><a href="https://jq.qq.com/?_wv=1027&k=zAXSPgxM" style="font-size:16px;color:red">点击进群</a></br><h3 style="font-size:18px">反馈QQ群--863397822</h3></br><h3 style="color:#FF3333;font-size:18px">失效后想长期使用请联系我们白嫖</h3></br><a href="http://faka.kaokaos.com" style="color:#FF0000;font-size:18px;">点击获取独享版</a></br><h3>公众号：小影搜题（防止失联）</h3><br></label>' +
        ...
        onload: function(xhr) {
            if (!setting.loop) {
                ...
                if (obj.answer) {
                    ...
                    setting.div.children('div:eq(0)').html(obj.data || setting.over + '服务器繁忙，正在重试...请联系vx:HH54068');
                }if(obj.msg) obj.msg='<h3>如果服务器繁忙请联系获取独享版本</h3>';
                 else obj.msg = '</br><h3 style="color:#FF3333;background-color:F5F5F5;font-size:18px;">反馈QQ群--863397822</h3></br><a href="http://faka.kaokaos.com" style="color:#FF3333";>点击获取授权</a><h3>搜题公众号：小影搜题</h3></br><h3 style="font-size:18px">失效后想长期使用请联系我们</h3></br>';
                setting.div.children('span').html(obj.msg || '');
            } else if (xhr.status == 403) {
                var html = xhr.responseText.indexOf('{') ? '请求过于频繁，建议稍后再试-请联系vx:HH54068' : $.parseJSON(xhr.responseText).data;
                setting.div.children('div:eq(0)').data('html', html).siblings('button:eq(0)').click();
            } else {
                setting.div.children('div:eq(0)').html(setting.over + '服务器异常，正在重试...请联系vx:HH54068');
            }
        },
        ontimeout: function() {
            setting.loop && setting.div.children('div:eq(0)').html(setting.over + '服务器超时，正在重试...请联系vx:HH54068');
        }
    });
}
